...
def generate_normal_salary(records: List[SalaryRecord], title: str, output_dir: str,
    ...
    add_explanation_sheet(wb, [
        ("正常工资薪金收入", [
            ...
            "本期收入 = 应发工资(ATC93AA 本次工资总额) − 独生子女费+采暖费(ATC936 本次免税) − 大病险（个人）(ATC93BD) − 补缴及退款保险差额（个人）(ATC93BE) + 交纳现金(ATC93X3 个人交纳现金) − 个人欠款(ATC93E)。",
...
def generate_formula_explanation_sheet(wb: Workbook, records: List[SalaryRecord]):
    ...
    w(5, 1, "本期收入 = 应发工资(本次工资总额 ATC93AA) − 独生子女费+采暖费(本次免税 ATC936) − 大病险（个人）(ATC93BD) − 补缴及退款保险差额（个人）(ATC93BE) + 交纳现金(个人交纳现金 ATC93X3) − 个人欠款(ATC93E)")
    w(6, 1, "说明：本期免税（独生子女费+采暖费，ATC936） = 采暖费(ATC93W21) + 独生子女费(ATC93W4)，经数据库验证 100% 吻合。")
    w(7, 1, "说明：个人交纳现金(ATC93X3)=个人现金交纳的社保（如零工资月份补缴），计入本期收入、对应五险一金列抵扣；个人欠款(ATC93E)为欠款冲抵（零工资补缴月份为负值，抵消累计补缴差额）。均与个税端回盘口径一致。")
    ...
    w(16, 1, "免税 = 本次免税(独生子女费+采暖费 ATC936) + 大病险（个人）(ATC93BD)")
    w(17, 1, "通俗理解（为什么这样加减）：")
    ...
    rel_rows = [
        ("", "【收入侧】", ""),
        ("应发工资(本次工资总额 ATC93AA)", "− 独生子女费+采暖费(本次免税 ATC936)", "免税不计收入"),
        ("", "− 大病险（个人）(ATC93BD)", "大病险个人部分同样免税"),
        ("", "− 补缴及退款保险差额（个人）(ATC93BE) + 交纳现金(X3) − 个人欠款(E)", "补缴/退款冲抵；现金交纳与欠款冲抵均计入收入"),
        ("= 本期收入", "← 报税口径的收入", ""),
        ...
        ("", "− 免税(独生子女费+采暖费+大病险个人)", "收入里已剔除的免税部分"),
        ("= 右", "← 从实发反推的同一笔钱", ""),
        ("", "", ""),
        ("左 = 右（差值 < 0.01）", "→ 数据正确", ""),
    ]
    ...
    rows = [
        ("本期收入", "应发工资(工资总额) − 独生子女费+采暖费(本次免税) − 大病险（个人） − 补缴及退款保险差额（个人） + 交纳现金(个人交纳现金) − 个人欠款", "ATC93AA, ATC936, ATC93BD, ATC93BE, ATC93X3, ATC93E"),
    ...
    if records:
        ...
        ex_rows = [
            ("姓名", rec.姓名, ""),
            ("应发工资(本次工资总额 ATC93AA)", rec.工资总额, ""),
            ("独生子女费+采暖费(本次免税 ATC936)", rec.补发3, ""),
            ("大病险（个人）(ATC93BD)", rec.大病险个人, ""),
            ("补缴及退款保险差额（个人）(ATC93BE)", rec.补缴及退款保险金额个人, ""),
            ("本期收入", income, "应发工资 − 独生子女费+采暖费 − 大病险 − 补缴退款差额 + 交纳现金 − 个人欠款"),
            ("本期免税收入", tax_exempt, "独生子女费+采暖费(ATC936) + 大病险（个人）(ATC93BD)，填报时列不填"),
            ...
            ("免税", tax_exempt, "独生子女费+采暖费(ATC936) + 大病险（个人）(ATC93BD)"),
