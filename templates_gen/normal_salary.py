...
def generate_normal_salary(records: List[SalaryRecord], title: str, output_dir: str,
    ...
    add_explanation_sheet(wb, [
        ...
        ("字段对应关系", [
            "工资表项目名称 → 数据库字段名称 → 数据库字段注释 三列对应表，同一数值在不同位置的名称一目了然。",
        ]),
    ])
...
def generate_field_mapping_sheet(wb: Workbook):
    """字段对应关系sheet：工资表项目名称 → 数据库字段名称 → 数据库字段注释，三列若干行。

    每行是同一个值在不同地方的名称：模板/报告列名、东软库字段名、字段注释。
    """
    ws = wb.create_sheet("字段对应关系")
    ws.column_dimensions["A"].width = 32
    ws.column_dimensions["B"].width = 20
    ws.column_dimensions["C"].width = 60
    headers = ["工资表项目名称", "数据库字段名称", "数据库字段注释"]
    for col, h in enumerate(headers, 1):
        ws.cell(row=1, column=col, value=h)
    rows = [
        ("本期收入", "计算字段", "工资总额 − 本次免税 − 大病险个人 − 补缴及退款保险差额（个人） + 个人交纳现金 − 个人欠款"),
        ("本期免税收入", "ATC936 + ATC93BD", "本次免税(ATC936) + 大病险个人(ATC93BD)，模板中该列不填"),
        ("本次免税", "ATC936", "采暖费(ATC93W21) + 独生子女费(ATC93W4)"),
        ("本次工资总额", "ATC93AA", "本次工资总额"),
        ("大病险（个人承担）", "ATC93BD", "大病险（个人承担）"),
        ("大病险（单位承担）", "ATC93BC", "大病险（单位承担），扣款-大病险 Y2=BD+BC"),
        ("补缴及退款保险差额（个人）", "ATC93BE", "补缴及退款保险差额（个人）"),
        ("个人交纳现金", "ATC93X3", "现金交纳社保等，计入本期收入"),
        ("个人欠款", "ATC93E", "欠款冲抵，计入本期收入（零工资补缴月份为负值）"),
        ("个人其他调整", "ATC93AG", "个人其他调整，实发已扣、右式加回复原"),
        ("当月养老个人缴", "BAA001", "8%"),
        ("当月医疗个人缴", "BAA002", "2%"),
        ("当月失业个人缴", "BAA003", "0.3%"),
        ("个人公积金月缴存额", "CAA002", "7%"),
        ("意外险（个人承担）", "ATC93BH", "意外险（个人承担）"),
        ("本次实发金额(合计)", "ATC93C", "本次实发金额(合计)"),
        ("税后扣除工会会费", "ATC93Z2", "税后扣除工会会费"),
        ("个人承担代理费", "BAA300", "个人承担代理费"),
        ("本次个人所得税", "ATC93D", "本次个人所得税"),
        ("本次经济补偿金", "ATC93M", "解除劳动合同一次性补偿，含在实发中，验证时从实发扣回"),
        ("基本养老保险费(模板)", "BAA001", "模板第7列 = 养老个人缴"),
        ("基本医疗保险费(模板)", "BAA002", "模板第8列 = 医疗个人缴"),
        ("失业保险费(模板)", "BAA003", "模板第9列 = 失业个人缴"),
        ("住房公积金(模板)", "CAA002", "模板第10列 = 公积金月缴存额"),
        ("工号", "AAC001", "个人编号"),
        ("姓名", "AAC003", "姓名"),
        ("证件号码", "AAC002", "公民身份号码"),
        ("工资所属年月", "ATC931", "工资属于哪个月"),
        ("工资发放年月", "ATC932", "TC93 表内字段"),
        ("经办年月", "ATC8G7", "TC8M 字段，发放月份下拉即此字段，以此判断申报期"),
        ("工资发放次数", "ATC937", "批次"),
        ("流水号", "ATC930", "TC93 主键流水号"),
        ("结算单元", "ATB930", "结算单元流水号"),
        ("结算单元名称", "ATB931", "结算单元名称"),
        ("转款合计", "ATC93BF", "转款合计"),
        ("补缴及退款个人所得税", "ATC93BG", "补缴及退款个人所得税"),
    ]
    for i, (c1, c2, c3) in enumerate(rows, 2):
