...
class TestTemplateGeneration:
    ...
    def test_generate_normal_salary(self, conn, output_dir):
        ...
        # 正常+合并+零申报 = 合计 = record_count; B类-欠费未发/次月发放为 B类子行
        # (2026-09-13 用户需求: 插入欠费未发子类, 零申报合计由 row8 下移至 row9)
        st = wb["申报分类统计"]
        normal = st.cell(row=2, column=3).value or 0
        merged = st.cell(row=3, column=3).value or 0
        zero = st.cell(row=9, column=3).value or 0
        total = st.cell(row=10, column=3).value or 0
        assert normal + merged + zero == len(records)
        assert total == len(records)
        non_zero = st.cell(row=11, column=3).value or 0
