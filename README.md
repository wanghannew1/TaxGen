...
- **跨月合并规则确认** — 跨月发放人员三险一金单月/多月/不报逐人确认（四场景数据判定 + 发薪模式配置 + 历史申报档案，含置信度提示、ATC93BE 借支强制单月）
...
> 业务规则与日常操作详见 `docs/`（操作指南、三险一金合并确认判定规则、合并规则分析、零申报规则、减员判定等）。
...
3. **检查合并规则** — 点「检查合并规则并确认」：处理跨月发放人员的三险一金**单月/多月/不报**，逐人确认后关闭
4. **检查零申报** — 点「检查零申报并确认」：确认本期收入为0人员是否**生成零申报**
5. **生成** — 勾选模板 → 「生成报税数据」，等待生成完成
6. **下载 & 验证** — 结果列表下载 4 类 Excel，查看验证通过/失败统计

推荐首次使用时先在「历史申报库」导入上个申报期的个税端导出文件（税款计算 xls），合并规则判定才更准确；不导入也可用（判定保守为单月）。
